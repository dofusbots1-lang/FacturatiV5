...
export default function Template2Modern({ data, type, includeSignature = false }: TemplateProps) {
  ...
  // Largeur utile & hauteurs (doivent matcher le CSS ci-dessous)
  const PAGE_W = 750;   // px ~ largeur utile A4
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        @media print {
          /* Header / Footer fixes et répétés */
          .print-header,
          .print-footer {
            position: fixed;
            left: 50%;
          ...
          /* Espace réservé pour éviter chevauchement */
          .print-body {
            margin-top: ${HEADER_H + 10}px;
            margin-bottom: ${FOOTER_H + 10}px;
          }

          /* Tableau propre à l'impression */
      ...
      {/* CONTENU (réserve l'espace du header/footer) */}
      ...
      <div className="print-footer bg-black text-white border-t-2 border-white p-4 text-xs text-center">
        <p>
          ...
          <strong> RC :</strong> {user?.company?.rc} | <strong> CNSS :</strong> {user?.company?.cnss} |
          <strong> Patente :</strong> {user?.company?.patente} | <strong> EMAIL :</strong> {user?.company?.email} |
