...
const Template2Modern = forwardRef<HTMLDivElement, TemplateProps>(
  ({ data, type, includeSignature = false }, ref) => {
    const { user } = useAuth();
            ...
            <div className="flex justify-between flex-wrap gap-4">
              ...
              <div className="w-80 bg-gray-50 border border-black rounded p-4 avoid-break">
                ...
                <div className="text-sm mb-2">
                  {(() => {
                    const g = data.items.reduce((acc: Record<number, number>, it) => {
                      const r = it.vatRate || 0;
                      const a = (it.unitPrice * it.quantity * r) / 100;
                      acc[r] = (acc[r] || 0) + a;
                      return acc;
                    }, {});
                    return Object.entries(g).map(([rate, amount]) => (
