...
export default function Template2Modern({ data, type, includeSignature = false }: TemplateProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="bg-black text-white p-4 text-center text-xs"
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-gray-50 p-4 rounded border border-black">
              <h3 className="font-bold text-sm text-black mb-2 border-b border-black pb-1 text-center">
                CLIENT
              </h3>
            ...
            <tbody>
              {data.items.map((item, index) => (
                <tr key={index} className="avoid-break-inside hover:bg-gray-50">
                  <td className="border border-black px-4 py-3 text-sm">{item.description}</td>
                  <td className="border border-black px-4 py-3 text-center text-sm">
                    {item.quantity.toFixed(3)} ({item.unit || 'unité'})
                  </td>
                  <td className="border border-black px-4 py-3 text-center text-sm">
                    {item.unitPrice.toFixed(2)} MAD
                  </td>
                  <td className="border border-black px-4 py-3 text-center font-medium text-sm">
